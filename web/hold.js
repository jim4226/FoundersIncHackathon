...
// Storage throws outright in a sandboxed iframe or with cookies fully blocked,
// and boot() reads it before requestAnimationFrame -- an unguarded read kills
// the render loop and leaves a black viewport frozen on "starting".
const readStore = (k) => { try { return localStorage.getItem(k); } catch (_) { return null; } };
const writeStore = (k, v) => { try { localStorage.setItem(k, v); } catch (_) { /* blocked */ } };
...
async function startCamera(deviceId) {
  ...
  if (state.deviceId) writeStore(CAM_KEY, state.deviceId);
...
async function boot() {
  ...
  state.mirror = readStore(MIRROR_KEY) !== 'off';
  ...
  if (params.has('sim')) {
    ...
    try {
      ...
      const label = await startCamera(params.get('cam') || readStore(CAM_KEY));
      await refreshCameras();
...
function setMirror(on) {
  state.mirror = on;
  writeStore(MIRROR_KEY, on ? 'on' : 'off');
}
